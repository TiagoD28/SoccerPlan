...
export default function Menu({userData}){
  ...
  useEffect(() => {
    ...
    const getNotifications = async () => {
        ...
        if(apiResponse.status == '200'){
          const sortedNotifications = apiResponse.data.sort((a, b) => {
            const timeA = new Date(a.timeExecuted);
            const timeB = new Date(b.timeExecuted);
  ...
  const handleAccept = async () => {
    console.log('Item: ', selectedNotification);
    dataToSend = {
      'idClub': selectedNotification.idClub,
      'idTeam': selectedNotification.idTeam,
      'idRequester': selectedNotification.idExecuter,
      'state': 'accepted'
    };

    if(selectedNotification.typeNotification == 'joinClub'){
      const apiResponse = await sendDataToApi('Requests', 'answerRequestClub', dataToSend);
      // console.log('Status: ',apiResponse.status);
      if(apiResponse.status == '200'){
    ...
    } else if(selectedNotification.typeNotification == 'joinTeam') {
      const apiResponse = await sendDataToApi('Requests', 'answerRequestTeam', dataToSend);
      // console.log('Status: ',apiResponse.status);
      if(apiResponse.status == '200'){
        showToast('success', 'Success', apiResponse.message);
      } else {
    ...
    if((item.idClub != dataUser.idClub) ||  // case user its not from the same club
      ...
      return (
        ...
        <View style={styles.iconContainer}>
          <Feather name={icon} color={hoursDifference < 15 ? '#f50443' : 'white'} size={25} style={styles.icon} />
        </View>
        {/* Section: Description in the middle */}
        <View style={styles.descriptionContainer}>
          {dataUser.idCoach && item.typeNotification === 'joinClub' || item.typeNotification === 'joinTeam' ? (
            <TouchableOpacity onPress={() => {
              setSelectedNotification(item);
              setShowModal(true);
            }}>
              <Text style={styles.titleRequest}>{title}</Text>
            </TouchableOpacity>
          ) : (
        ...
        <View>
          ...
          {hoursDifference < 1
            ...
            : `${timeExecuted.toLocaleString('default', { day: 'numeric', month: 'short' })}`}
          </Text>
        </View>
      </View>
      );  
